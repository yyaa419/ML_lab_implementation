# AIM: Explore the role of the slack parameter C
import numpy as np
...
classA=q4_data.classA
classB=q4_data.classB
target=q4_data.target
input=q4_data.input
...
for i in range(0,n):
    for j in range(0,n):
        p_ij[i][j]=target[i]*target[j]*k.polynomial_kernel(input[i],input[j])
...
for index in range(0,n_slected):
    sum += alpha_selected[index]*target_selected[index]*k.polynomial_kernel(s,datapoint_selected[index])
